...
$request_id = $this->db->insert_id();
    ...
    function create_act(){
        ...
        // Check if loan creation failed due to duplicate or other error
        if (isset($result['status']) && $result['status'] === 'error') {
            $this->toaster->error($result['message']);
            redirect('loan/loan_application');
            return;
        }
...
// Create directory if it doesn't exist
        ...
        // Show success message only if loan was created successfully
...
//  $scores = $this->Loan_recommendation_model->get_by_loan($row->loan_id);
	...
	/**
	 * AJAX endpoint to check for duplicate loans
	 */
	function check_duplicate() {
		...
		// Validate input
		if (empty($customer) || empty($amount) || empty($loan_date)) {
		...
		// Check for duplicate
		$duplicate = $this->Loan_model->check_duplicate_loan($customer, $loan_date, $amount);

		$response = array('is_duplicate' => false);

		if ($duplicate) {
			// Get customer name for better user experience
			$customer_info = null;
			if ($duplicate->customer_type === 'individual') {
				$customer_info = $this->Individual_customers_model->get_by_id($customer);
				$customer_name = isset($customer_info->Firstname) ? $customer_info->Firstname . ' ' . $customer_info->Lastname : 'Unknown Customer';
			} elseif ($duplicate->customer_type === 'institution') {
				$customer_info = $this->Corporate_customers_model->get_by_id($customer);
				$customer_name = isset($customer_info->EntityName) ? $customer_info->EntityName : 'Unknown Institution';
			} else {
			...
			$response = array(
				...
				'loan_number' => $duplicate->loan_number,
				'loan_date' => $duplicate->loan_date,
				'loan_amount' => $duplicate->loan_principal,
				'message' => 'This will be duplicate. Change values. Loan already exists with same customer, loan date, and amount.'
			);
		}
