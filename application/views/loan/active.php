...
<div class="main-content">
    <div class="page-header">
        <h2 class="header-title">Active Loans (Disbursed)</h2>
        <div class="header-sub-title">
            <nav class="breadcrumb breadcrumb-dash">
                <a href="<?php echo base_url('Admin')?>" class="breadcrumb-item"><i class="anticon anticon-home m-r-5"></i>Home</a>
                <a class="breadcrumb-item" href="#">Loans</a>
                <span class="breadcrumb-item active">Active Loans</span>
            </nav>
        </div>
    </div>
    <div class="card">
        <div class="card-body" style="border: thick #153505 solid;border-radius: 14px;">
            ...
            <table id="data-table" class="tableCss">
                ...
                foreach ($loan_data as $loan) {
                    ...
                    } elseif (isset($loan->customer_type) && $loan->customer_type == 'institution') {
                        $corporate = get_by_id('corporate_customers', 'id', $loan->loan_customer);
                        if ($corporate) {
                            $customer_name = $corporate->EntityName;
                            $customer_link = base_url('corporate_customers/read/' . $corporate->id);
                        } else {
                            $customer_name = 'Institution';
                        }
                    } elseif (isset($loan->customer_type) && $loan->customer_type == 'group') {
                        $group = get_by_id('groups', 'id', $loan->loan_customer);
                        if ($group) {
                            $customer_name = $group->group_name;
                            $customer_link = base_url('groups/view/' . $group->id);
                        } else {
                            $customer_name = 'Group';
                        }
                    } else {
                ...
                ?>
                    <tr>
                        ...
                        <td><span class="badge badge-success"><?php echo $loan->loan_status ?></span></td>
                        <td><?php echo $loan->loan_added_date ?></td>
                        <td>
                            <a href="<?php echo base_url('loan/repayment_view/' . $loan->loan_id); ?>" class="btn btn-sm btn-primary">
