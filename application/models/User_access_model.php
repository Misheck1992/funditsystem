...
class User_access_model extends CI_Model
{
    ...
    function check_user_email($user){
        $this->db->select("*");
        $this->db->from($this->table);
        $this->db->join('employees','employees.id=user_access.Employee');
        $this->db->join('roles','roles.id=employees.Role');
        $this->db->where('employees.EmailAddress',$user);
        return $this->db->get()->row_array();
    }

    function check_user_code($code, $id,$acesscode){
        $this->db->select("*");
        $this->db->from($this->table);
    ...
    // get data by id
    function get_by_id_use_email($id)
    {
        $this->db->select("*");
        $this->db->from($this->table);
        $this->db->join('employees','employees.id=user_access.Employee');
        $this->db->join('roles','roles.id=employees.Role');
        $this->db->where('employees.EmailAddress',$id);
        return $this->db->get()->row();
    }
