...
class Shareholders_model extends CI_Model
{
    ...
    public function insert_batch($data) {
        $inserted_ids = [];
        foreach ($data as $row) {
            $this->db->insert('shareholders', $row);
            $inserted_ids[] = $this->db->insert_id(); // Capture insert ID
        }
    ...
    function get_all()
    {
        $this->db->order_by('shareholders.id', $this->order)
        ->join('corporate_shareholders','corporate_shareholders.shareholder_id=shareholders.id');
    ...
    function get_selective($id)
    {
        ...
        $this->db->where('entity_type','client');
        return $this->db->get($this->table)->result();
    }
    ...
    function total_rows($q = NULL) {
        ...
        $this->db->or_like('full_address', $q);
